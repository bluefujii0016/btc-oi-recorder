...
def load_state():
    if os.path.exists(STATE_PATH):
        with open(STATE_PATH, "r", encoding="utf-8") as f:
            return json.load(f)
    return {"last_notified_flip_t": None}
...
def main():
    ...
    # 転換1点目 かつ 未通知の場合のみ通知
    is_new_flip = latest["reversed"] and (state.get("last_notified_flip_t") != latest["t"])

    if is_new_flip:
        notify_discord(record)
        state["last_notified_flip_t"] = latest["t"]
        save_state(state)

    # ログは毎回蓄積(dots_since_flipの推移を追えるように)
    append_log(record)

    print(json.dumps(record, ensure_ascii=False, indent=2))
    if is_new_flip:
        print("=> 転換1点目を検知し、Discordに通知しました", file=sys.stderr)
